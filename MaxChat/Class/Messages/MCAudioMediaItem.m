...
@interface MCAudioMediaItem ()

@property (nonatomic, strong) JSQAudioMediaViewAttributes *audioViewAttributes;
...
@implementation MCAudioMediaItem
...
- (UIView *)mediaView
{
    if (self.audioData && self.cachedMediaView == nil) {
        ...
        CGRect buttonFrame = CGRectMake(self.audioViewAttributes.controlInsets.left + 8,
                                        self.audioViewAttributes.controlInsets.top,
...
- (BOOL)isEqual:(id)object
{
    ...
    MCAudioMediaItem *audioItem = (MCAudioMediaItem *)object;
    if (self.audioData && ![self.audioData isEqualToData:audioItem.audioData]) {
        return NO;
    }
    
    if (self.audioURL && ![self.audioURL.path isEqualToString:audioItem.audioURL.path]) {
        return NO;
    }
...
{
    MCAudioMediaItem *copy = [[[self class] allocWithZone:zone] initWithData:self.audioData
                                                          audioViewAttributes:self.audioViewAttributes];
    [copy setAudioDataWithUrl:self.audioURL];
